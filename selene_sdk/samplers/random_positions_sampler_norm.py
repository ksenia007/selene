...
class RandomPositionsSamplerNorm(OnlineSampler):
    ...
    def __init__(self,
                 ...
                 norm_params = None): #[min, max]
        super(RandomPositionsSamplerNorm, self).__init__(
            reference_sequence,
            target_path,
            features,
            seed=seed,
        ...
        self.norm_params = np.array(norm_params)

        if self.norm_params.shape[0] != len(self.all_bw_files):
            raise Exception('Number of normalization params is different from number of files')
    ...
    def _retrieve(self, chrom, position):
        ...
        # retrieve additonal information from BW files AND normalize
        if (not self.all_bw_files is None) and len(self.all_bw_files)>0:
            ...
            for i, score_file in enumerate(self.all_bw_files):
                ...
                # normalize
                score_array = (score_array - self.norm_params[i][0])/(self.norm_params[i][1] - self.norm_params[i][0])
