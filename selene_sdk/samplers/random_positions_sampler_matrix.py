"""
This module provides the RandomPositionsSamplerMatrix class.
...
class RandomPositionsSamplerMatrix(OnlineSampler):
    ...
    The output is of the shape [1 x 4 x 2000] is there are no additional files.
    
    TO DO: if there are additional files, preferrably 4, make it [2 x 4 x 2000]?
    """
    def __init__(self,
                 reference_sequence,
                 target_path,
        ...
        super(RandomPositionsSamplerMatrix, self).__init__(
            reference_sequence,
            target_path,
            features,
            seed=seed,
    ...
    def sample(self, batch_size=1):
        ...
        """
        sequences = np.zeros((batch_size, 1, 4, self.sequence_length))
        ...
        while n_samples_drawn < batch_size:
            ...
            sequences[n_samples_drawn,:, :, :] = np.expand_dims(np.transpose(seq), axis=0)
